...
export const saveToStoreProcedure = publicProcedure
  ...
  .mutation(async ({ input }) => {
    ...
    const exportData = clients.map((client) => ({
      ...
      'Tiene Crédito': client.hasCredit ? 'Sí' : 'No',
      'Días de Crédito': client.creditDays || 'N/A',
      'Tiene Descuento': client.hasDiscount ? 'Sí' : 'No',
      'Descuento (MXN)': client.discountAmount || 'N/A',
      'Notas': client.notes,
    ...
    const clients = data
      ...
      .map((row: any) => {
        ...
        return {
          ...
          area: row['Área'] || '',
          hasCredit: row['Tiene Crédito'] === 'Sí',
          creditDays: row['Días de Crédito'] && row['Días de Crédito'] !== 'N/A' ? Number(row['Días de Crédito']) : undefined,
          hasDiscount: row['Tiene Descuento'] === 'Sí',
          discountAmount: row['Descuento (MXN)'] && row['Descuento (MXN)'] !== 'N/A' ? Number(row['Descuento (MXN)']) : undefined,
